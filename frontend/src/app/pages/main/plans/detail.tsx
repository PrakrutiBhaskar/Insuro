...
export default function PlanDetail() {
  ...
  const dynamicPlan = store.modelResult?.top_plans?.find((p: any) => p.plan_id === id);
  const staticPlan = allPlans.find(p => p.id === id);

  const plan = dynamicPlan ? {
    id: dynamicPlan.plan_id,
    name: dynamicPlan.name,
    ins: dynamicPlan.insurer,
    suitability: dynamicPlan.suitability_score ? `${(dynamicPlan.suitability_score * 100).toFixed(0)}%` : 'N/A',
    price: dynamicPlan.monthly_premium_inr || staticPlan?.price || 0,
    sumInsured: dynamicPlan.sum_insured_inr ? `₹${(dynamicPlan.sum_insured_inr/100000).toFixed(0)}L` : staticPlan?.sumInsured || 'N/A',
    deductible: dynamicPlan.features?.deductible ? `₹${dynamicPlan.features.deductible}` : staticPlan?.deductible || 'Standard',
    cashlessNetwork: dynamicPlan.features?.cashless ? 'Available' : staticPlan?.cashlessNetwork || 'Network Hospitals',
    opdCover: dynamicPlan.features?.opd_cover ? 'Covered' : staticPlan?.opdCover || 'Not Covered',
    diabetesOpd: dynamicPlan.features?.diabetes_opd ? 'Covered' : staticPlan?.diabetesOpd || 'Not Covered',
    preventive: dynamicPlan.features?.preventive ? 'Covered' : staticPlan?.preventive || 'Not Covered',
    mentalHealth: dynamicPlan.features?.mental_health ? 'Covered' : staticPlan?.mentalHealth || 'Not Covered',
    benefits: dynamicPlan.features?.benefits || staticPlan?.benefits || ['Comprehensive hospitalization', 'Pre and post hospitalization', 'Day care procedures'],
    exclusions: dynamicPlan.features?.exclusions || staticPlan?.exclusions || ['Pre-existing diseases (waiting period applies)', 'Cosmetic surgery'],
    pros: dynamicPlan.explanation || staticPlan?.pros || 'Matches your health profile well.',
    cons: staticPlan?.cons || 'Premiums may increase with age or claims.',
  } : staticPlan;
  ...
  if (!plan) {
    return (
      <div className="screen active" style={{ padding: '40px', textAlign: 'center' }}>
        ...
        <button className="btn btn-em" onClick={() => navigate('/plans')}>Back to Recommendations</button>
      </div>
    );
  }
           ...
           <button 
             ...
             onClick={() => {
               confetti({
                 ...
                 spread: 70,
                 origin: { y: 0.6 },
                 colors: ['#00e5a0', '#ffffff', '#7CB8E0']
               });
               window.dispatchEvent(new CustomEvent('show-toast', { detail: 'Application started successfully! An agent will contact you shortly.' }));
             }}
           >
             Start Application
           </button>
