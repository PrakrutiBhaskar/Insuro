  ...
  const handleSubmit = async () => {
    try {
      ...
      if (tab === "signin") {
        ...
        login(res.user.email, res.user.name, res.token);
      } else {
        ...
        const loginRes = await api.auth.login(email || "demo@example.com", password || "demo");
        login(res.user.email, res.user.name, loginRes.token);
      }
      navigate("/onboarding");
    } catch (e: any) {
      console.error(e);
